export const data = [
  {
    id: 1,
    type: "dumbbells",
    title: "Rogue Dumbbells",
    img: "https://assets.roguefitness.com/f_auto,q_auto,c_limit,w_1600,b_rgb:ffffff/catalog/Conditioning/Strength%20Equipment/Dumbbells/XX7125/XX7125-H_u6c9kv.png",

    price: 20,

    singlePage: {
      decTitle: "ROGUE RUBBER HEX DUMBBELLS",
      dec: `Rogue Dumbbells are sold in pairs and available in weights ranging from 2.5LB to 65LB. This standard rubber hex design is a great, budget-priced option that still delivers on quality, performance, and longevity. The heavy-duty, rubber-encased heads minimize noise and limit wear and tear (to both the bells and your flooring), while the chrome-plated handle is ergonomically designed for a firm but comfortable feel in any grip style.

All smaller Rogue Dumbbells up to 10LB feature a 25MM diameter handle, while those 12.5LB and up have a larger 35MM diameter handle.

Dumbbell training has stood the test of time for good reason. Along with the versatility in weight choices for multiple athletes, a good set of bells can be used to duplicate nearly all types of barbell exercises, while also offering a wider range of single-arm, rehab-specific, and explosive movements in virtually any sized space—something not possible with bars or most weight room machines.`,
      Specifications: `Weight range: 2.5LB to 65LB
Tolerance: +/- 3%
Handle Diameter: 25MM up to 10LB, 35MM for 12.5LB and above
Heavy-duty Rubber Encased Heads: minimize noise, floor damage, and wear and tear on the dumbbells themselves.
Easy-grip, ergonomic Chrome-Plated Handle
Proprietary head to handle construction ensures the head does not come loose`,
      imgs: [
        "https://assets.roguefitness.com/f_auto,q_auto,c_limit,w_1600,b_rgb:ffffff/catalog/Conditioning/Strength%20Equipment/Dumbbells/XX7125/XX7125-H_u6c9kv.png",
        "https://storagecdn2.myecomz.com//assets/kore/Product/product-dumbells-web-637481945969644583.webp",
        "https://cdn.shopify.com/s/files/1/0465/3596/6881/products/MAGMA-5-25LB-DumbbellSet-MAGMA-Knurl_600x.jpg?v=1647401894",
        "https://i.pinimg.com/736x/ce/f1/b9/cef1b9eca8258928a6460f1d3e2294b9.jpg",
        "https://cdn.shopify.com/s/files/1/2350/9323/products/ritfit-rubber-hex-dumbbell-set-10-60-lbs-weight-ritfit-670542.jpg?v=1653981509",
      ],
    },
  },
  {
    id: 2,
    type: "dumbbells",
    title: "Rogue Loadable Dumbbells",
    img: "https://assets.roguefitness.com/f_auto,q_auto,c_limit,w_1600,b_rgb:ffffff/catalog/Conditioning/Strength%20Equipment/Dumbbells/LOADABLEDBS/LOADABLEDBS-WEB1_zy97cf.png",
    price: 200,

    singlePage: {
      decTitle: "",
      dec: "",
      imgs: ["", "", "", "", ""],
    },
  },
  {
    id: 3,
    type: "barbells",
    title: "The Ohio Bar - Cerakote",
    img: "https://assets.roguefitness.com/f_auto,q_auto,c_limit,w_1600,b_rgb:ffffff/catalog/Weightlifting%20Bars%20and%20Plates/Barbells/Mens%2020KG%20Barbells/OHIOCERAKOTE/Updated%20Headers%201-25-22/OHIOCERAKOTE_0010_red-black_sfvapn.png",
    price: 200,

    singlePage: {
      decTitle: "",
      dec: "",
      imgs: ["", "", "", "", ""],
    },
  },
  {
    id: 4,
    type: "barbells",
    title: "The Ohio Bar - Cerakote",
    img: "https://assets.roguefitness.com/f_auto,q_auto,c_limit,w_1600,b_rgb:ffffff/catalog/Weightlifting%20Bars%20and%20Plates/Barbells/Mens%2020KG%20Barbells/OHIOCERAKOTE/Updated%20Headers%201-25-22/OHIOCERAKOTE_0010_red-black_sfvapn.png",
    price: 200,

    singlePage: {
      decTitle: "",
      dec: "",
      imgs: ["", "", "", "", ""],
    },
  },
  {
    id: 5,
    type: "barbells",
    title: "The Ohio Bar - Cerakote",
    img: "https://assets.roguefitness.com/f_auto,q_auto,c_limit,w_1600,b_rgb:ffffff/catalog/Weightlifting%20Bars%20and%20Plates/Barbells/Mens%2020KG%20Barbells/OHIOCERAKOTE/Updated%20Headers%201-25-22/OHIOCERAKOTE_0010_red-black_sfvapn.png",
    price: 200,

    singlePage: {
      decTitle: "",
      dec: "",
      imgs: ["", "", "", "", ""],
    },
  },
  {
    id: 6,
    type: "barbells",
    title: "The Ohio Bar - Cerakote",
    img: "https://assets.roguefitness.com/f_auto,q_auto,c_limit,w_1600,b_rgb:ffffff/catalog/Weightlifting%20Bars%20and%20Plates/Barbells/Mens%2020KG%20Barbells/OHIOCERAKOTE/Updated%20Headers%201-25-22/OHIOCERAKOTE_0010_red-black_sfvapn.png",
    price: 200,

    singlePage: {
      decTitle: "",
      dec: "",
      imgs: ["", "", "", "", ""],
    },
  },
];

export default data;

/*{
    type: " ",
    title: " ",
    img: "",
    price: "$200.00",

    singlePage: {
      decTitle: "",
      dec: "",
      imgs: {
        img1: "",
        img2: "",
        img3: "",
        img4: "",
        img5: "",
      },
    },
  },*/
